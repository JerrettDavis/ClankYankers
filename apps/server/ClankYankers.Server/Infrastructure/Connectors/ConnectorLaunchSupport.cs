...
internal static class ConnectorLaunchSupport
{
    ...
    public static string? ResolvePermissionMode(ConnectorDefinition definition) =>
        CoalesceValue(definition.DefaultPermissionMode);

    public static IReadOnlyList<string> ResolveAllowedTools(ConnectorDefinition definition) =>
        definition.AllowedTools
            .Select(tool => tool.Trim())
            .Where(tool => tool.Length > 0)
            .ToArray();

    public static bool ResolveSkipPermissions(ConnectorDefinition definition) => definition.SkipPermissions;
